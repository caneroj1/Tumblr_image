...
class Window < Gosu::Window 
  ...
  def draw
    @font.draw("#{@fr}", 100, 100, 0, 5, 5, 0xFFFFFFFF, :default)
  end
end
...
class TextInputWindow < Gosu::Window
  def initialize
    super(300, 200, false)
    self.caption = "Enter URL"
    
    font = Gosu::Font.new(self, Gosu::default_font_name, 20)
    
    # Set up a text field
    @text_field = TextField.new(self, font, 50, 80)
  ...
  def button_down(id)
    if id == Gosu::MsLeft then
      ...
      if @text_field.under_point?(mouse_x, mouse_y) then
